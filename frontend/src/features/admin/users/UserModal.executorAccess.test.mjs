...
test("user form uses executor access instead of legacy profile", async () => {
  ...
  assert.doesNotMatch(adminPage, legacyScope);
  assert.doesNotMatch(adminPage, legacyProfile);
  assert.doesNotMatch(userModal, /MASTER_SCOPE_OPTIONS/);
  assert.doesNotMatch(userModal, legacyScope);
  assert.match(adminPage, /executor_access:\s*accessPayloadFromForm\(userForm\.executorAccess\)/);
