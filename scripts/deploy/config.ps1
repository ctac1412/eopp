...
# .env.deploy is now in project_root/prod/
$EnvFile = Join-Path (Join-Path $ProjectRoot "prod") ".env.deploy"
...
$script:LocalBackupDir = if ($env:LOCAL_BACKUP_DIR) { $env:LOCAL_BACKUP_DIR } else { (Join-Path (Join-Path $ProjectRoot "prod") "backups") }
