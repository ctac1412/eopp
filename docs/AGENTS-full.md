...
## Codex Risk-Based TDD Mode

Use risk-based development speed by default. TDD remains required for changes
where a regression is expensive; low-risk work should use a short
implementation-plus-focused-verification loop instead of full red-green-refactor
ceremony.

Start future Codex implementation prompts with this instruction when speed
matters:

```text
Сделай задачу в risk-based режиме.
TDD нужен только если изменение затрагивает protected core, backend business logic, DB, billing/security, realtime fanout или исправляет баг с риском регрессии.
Если это UI/CSS/config/docs/prototype, не делай red-green-refactor; сделай scoped implementation и focused verification.
Перед завершением кратко скажи, какие проверки запускал и почему их достаточно.
```

Keep test-first TDD for:
- `server/src/core/*`
- RBAC, audit, security, permissions, and access policy changes
- billing, CRM, usage accounting, tariffs, invoices, prepaid, and finance logic
- database migrations, repositories, and persistence contracts
- bug fixes that need a regression test
- realtime fanout, captcha hot path, shared runtime, and protected-core contracts

Do not require test-first TDD for:
- CSS, layout, visual polish, copy, tabs, and minor navigation
- config, scripts, docs, prompts, and local tooling changes
- throwaway prototypes and performance diagnostics
- changes with a clearer manual, browser, build, or focused-check verification

For fast-path work:
- First understand the affected area through CodeGraph or targeted reads.
- Make the smallest scoped change.
- Run one relevant focused check, build, or browser verification.
- Avoid full-suite runs unless the touched layer or failure mode justifies them.

For large features, use thin-slice TDD: one failing test for the key contract,
then a minimal vertical implementation, then extra tests only for real edge
