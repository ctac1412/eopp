...
const AuthGate = React.memo(function AuthGate({ onClose }: Props) {
  ...
  return (
    <div className="injector-auth-gate-overlay">
      <button className="injector-auth-gate-close" onClick={onClose}>&times;</button>
      <div className="injector-auth-gate-card">
        <div className="injector-auth-gate-title">EOPP Injector</div>
        <div className="injector-auth-gate-subtitle">Введите API ключ для начала работы</div>
        <form className="injector-auth-gate-form" onSubmit={handleLogin}>
          <input
            ...
            autoFocus
          />
          {authError && <div className="injector-auth-gate-error">{authError}</div>}
          <button
        ...
        {authKey && (
          <div className="injector-auth-gate-logged">
            ...
            <button className="injector-auth-gate-logout" onClick={handleLogout}>Выйти</button>
          </div>
        )}
      </div>
    </div>
