...
def main(
    ...
    typer.echo("  GET  /api/stream    — SSE stream for new captchas")
    typer.echo("  POST /api/solve-captcha — Submit captcha (blocks until solved)")
    typer.echo("  POST /api/solve     — Submit solution (called by UI)")
    typer.echo("  POST /api/trigger-test — Trigger test cases")
    typer.echo("  POST /api/broadcast  — Broadcast SSE event")
    typer.echo()
