...
DB_PATH = os.environ.get(
    "EOPP_DB_PATH",
    os.path.join(PROJECT_DIR, "server", "data", "api_keys.db"),
)
CAPTCHA_DIR = os.environ.get(
    "EOPP_CAPTCHA_DIR",
    os.path.join(PROJECT_DIR, "server", "data", "captcha_examples", "all"),
)
_MODELS_DIR = os.path.join(os.path.dirname(__file__), "..", "..", "models")

MODEL_PATH = os.environ.get(
    "EOPP_TILE_MODEL_PATH",
    os.path.join(_MODELS_DIR, "tile_classifier.pkl"),
)
...
def save_model(scaler: StandardScaler, clf: LinearSVC, name: str = "hog_svm") -> str:
    """Save model to server/models/{name}_v{version}.pkl, auto-incrementing version."""
    ...
    default_path = os.path.join(MODELS_DIR, "tile_classifier.pkl")
...
def list_models() -> list[dict]:
    """List all trained models in server/models/."""
    os.makedirs(MODELS_DIR, exist_ok=True)
    models = []
