...
def test_invoice_list_includes_ledger_operator_executor_profit_calculation(isolated_api_db):
    ...
    conn.execute(
        ...
        ("op-profit", _now(), 10, "custom", company_id),
    )
    ...
    entries = [
        ...
        ("executor_salary", -100, executor_user_id, "executor"),
        ("operator_salary", -10, operator_user_id, "operator:1"),
        ("operator_salary", -10, operator_user_id, "operator:2"),
        ("operator_salary", -10, operator_user_id, "operator:3"),
        ("operator_salary", -10, operator_user_id, "operator:4"),
    ]
    for kind, amount, user_id, suffix in entries:
        conn.execute(
            """
            INSERT INTO finance_entries
                (company_id, invoice_id, usage_log_id, kind, amount, user_id, edit_state, source, source_key, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, 'open', 'test', ?, ?, ?)
            """,
            (company_id, invoice_id, usage_log_id, kind, amount, user_id, f"profit-side:{suffix}", _now(), _now()),
        )
...
def test_invoice_profit_side_payments_use_finance_entries_when_present(isolated_api_db):
    from src.db.connection import get_connection
    from src.db.invoices import list_invoices

    conn = get_connection()
    conn.execute("INSERT INTO companies (name, created_at) VALUES (?, ?)", ("Ledger Side Co", _now()))
    company_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    conn.execute(
        "INSERT INTO users (name, role, active, is_director, created_at) VALUES (?, ?, ?, ?, ?)",
        ("Executor", "manager", 1, 0, _now()),
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        ("INV-LEDGER-SIDE-1", "Ledger Side Co", 0, 3000, 150, 120, 3000, _now()),
    )
    invoice_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    conn.execute(
        "INSERT INTO api_keys (key, label, created_at, company_id) VALUES (?, ?, ?, ?)",
        ("ledger-side-key", "Ledger Side", _now(), company_id),
    )
    api_key_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    conn.execute(
        """
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (api_key_id, "res-ledger-side", "confirmed", _now(), 3000, 0, invoice_id, company_id),
    )
    usage_log_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    entries = [
        ("customer_income", 3000, None, "income"),
        ("executor_salary", -500, executor_user_id, "executor"),
        ("operator_salary", -75, operator_user_id, "operator:1"),
        ("operator_salary", -75, operator_user_id, "operator:2"),
        ("operator_salary", -75, operator_user_id, "operator:3"),
    ]
    for kind, amount, user_id, suffix in entries:
        conn.execute(
            """
            ...
            VALUES (?, ?, ?, ?, ?, ?, 'open', 'test', ?, ?, ?)
            """,
            (company_id, invoice_id, usage_log_id, kind, amount, user_id, f"ledger-side:{suffix}", _now(), _now()),
        )
    conn.commit()
    conn.close()
    ...
    assert invoice["operator_icons"] == 3
    assert invoice["operator_amount"] == 225.0
    assert invoice["executor_count"] == 1
    assert invoice["executor_amount"] == 500.0
    assert invoice["side_payout_amount"] == 725.0
    assert invoice["profit_amount"] == 2275.0


def test_invoice_side_payments_do_not_fallback_to_distribution_answers(isolated_api_db):
    from src.db.connection import get_connection
    from src.db.invoices import list_invoices

    conn = get_connection()
    conn.execute("INSERT INTO companies (name, created_at) VALUES (?, ?)", ("Ledger Only Co", _now()))
    company_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    conn.execute(
        "INSERT INTO users (name, role, active, is_director, created_at) VALUES (?, ?, ?, ?, ?)",
        ("Operator", "operator", 1, 0, _now()),
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        (company_id, 1000, 700, 100, 75, _now(), _now()),
    )
    conn.execute(
        "INSERT INTO operators (uuid, created_at, icon_rate, billing_mode, company_id) VALUES (?, ?, ?, ?, ?)",
        ("op-ledger-only", _now(), 0, "company", company_id),
    )
    operator_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    conn.execute(
        "INSERT INTO operator_profiles (user_id, company_id, operator_id, active, created_at) VALUES (?, ?, ?, 1, ?)",
    ...
    conn.execute(
        "INSERT INTO api_keys (key, label, created_at, company_id) VALUES (?, ?, ?, ?)",
        ("ledger-only-key", "Ledger Only", _now(), company_id),
    )
    api_key_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    conn.execute(
        """
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        ("INV-LEDGER-ONLY-1", "Ledger Only Co", 0, 1000, 0, 0, 1000, _now()),
    )
    invoice_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    conn.execute(
        """
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (api_key_id, "res-ledger-only", "confirmed", _now(), 1000, 0, invoice_id, company_id),
    )
    usage_log_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    conn.execute(
        "INSERT INTO captchas (captcha_id, status, usage_log_id, created_at) VALUES (?, ?, ?, ?)",
        ("captcha-ledger-only", "passed", usage_log_id, _now()),
    )
    for icon_position in range(2):
        conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (1, usage_log_id, "captcha-ledger-only", operator_id, icon_position, 10, 20, _now()),
        )
    conn.commit()
    conn.close()
    ...
    assert invoice["operator_icons"] == 0
    assert invoice["operator_amount"] == 0.0
    assert invoice["executor_count"] == 0
    assert invoice["executor_amount"] == 0.0
    assert invoice["side_payout_amount"] == 0.0
    assert invoice["profit_amount"] == 1000.0
